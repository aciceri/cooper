...
\subsection{Complessità computazionale} Si accenni solamente al notevole
risultato dovuto a Fischer e Rabin\autocite{complexity}, nel \date{1974}
mostrarono infatti che data $n$ la lunghezza di una formula nell'aritmetica di
Presburger, ogni problema decisionale ha complessità temporale $2^{2^{cn}}$ nel
caso peggiore, per qualche costante $c \ge 0$. Ovvero l'algoritmo di Cooper è
obbligatoriamente NP-difficile avendo una complessità almeno esponenziale
doppia.
...
\subsection{Il programma \inline{test.c}} Si consideri il seguente programma di
esempio contenuto in \inline{test.c}:
