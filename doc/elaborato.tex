...
\subsection{Il programma \inline{test.c}}
...
\'E semplice immaginare cosa facciano le regole \inline{run}, \inline{valgrind},
\inline{debug} e \inline{clean}. Ci si soffermi ora su \inline{eval} e
\inline{sat}. La prima esegue semplicemente \inline{test} con la formula in
...
soddsfacibilità si usi invece la regola \inline{sat} del \inline{makefile}.
Tale regola esegue il seguente script Python\autocite{python}.

\inputminted[linenos]{python}{../sat.py}

Tale script genera un opportuno sorgente \inline{source.ys} per
Yices\autocite{yices} e successivamente lo esegue, per esempio se la regola
\inline{make sat} esegue \inline{./sat.py "(and (> (+ (* 2 x) (* 3 y)) 1))" "x
  y"  "x"} allora viene generato il seguente \inline{source.ys} che viene poi
eseguito da Yices che restituisce la stringa \inline{"sat"}.
...
una formula senza la variabile $x$ che a sua volta \inline{Yices} dice essere
